...
img = cv.imread('screens/sample-6.png')
template = cv.imread('assets/esteh-1.png', 0)
...
while True:
    ...
    frame_result = cv.matchTemplate(
        frame_threshold, template, cv.TM_CCOEFF_NORMED)

    _, max_val, _, max_loc = cv.minMaxLoc(frame_result)
